...
async function discoverTopics(expertise: string[], recentTopics: string[], lang: string): Promise<string[]> {
  ...
  var result = await claude(
    ...
    "\n\nJedes Thema MUSS:" +
    "\n1. SPEZIFISCH sein - nicht 'KI im Business' sondern 'Warum die meisten KI-Projekte im Mittelstand an der IT-Abteilung scheitern'" +
    "\n2. Eine MEINUNG provozieren - Leser muessen widersprechen oder zustimmen wollen" +
    "\n3. AKTUELL sein (" + currentMonth + " - referenziere aktuelle Entwicklungen, Quartalsthemen, saisonale Aspekte)" +
    "\n4. Eine PERSOENLICHE GESCHICHTE ermoeglichen" +
    avoidStr +
    "\n\nMix: 2x kontraere Meinung, 2x persoenliche Erfahrung, 1x praktischer Tipp, 1x Branchenbeobachtung" +
...
async function critiqueAndPolish(draft: string, authorName: string, expertise: string): Promise<{ polished: string; hooks: string[]; score: number }> {
  var critiquePrompt = "Hier ist ein LinkedIn-Post-Entwurf von " + authorName + " (Experte fuer " + expertise + "):\n\n" +
    ...
    "POLISHED_POST_START\n[Der verbesserte Post. Behalte was gut ist, fixe die Schwaechen. Gleiche Laenge.]\nPOLISHED_POST_END\n\n" +
    "HOOK_ALT_1_START\n[Alternative erste 2 Zeilen - provokanter]\nHOOK_ALT_1_END\n\n" +
    "HOOK_ALT_2_START\n[Alternative erste 2 Zeilen - persoenlicher/Story-Einstieg]\nHOOK_ALT_2_END";
