...
export async function POST(request: NextRequest) {
  try {
    ...
    var draftData = draftRes.data;
 
    if (!draftData || draftData.length === 0) {
      return NextResponse.json({ error: "Draft not found" }, { status: 404 });
    }
 
    var draft = draftData[0];
    ...
    if (action === "suggest") {
      ...
      await supabase.rpc("update_draft_image", { p_draft_id: draftId, p_image_url: draft.image_url || "", p_image_source: draft.image_source || "" });
    ...
    if (action === "generate-card") {
      ...
      var svg = generateSVGCard(headline, "");
      ...
      // Use RPC to update draft image
      await supabase.rpc("update_draft_image", { p_draft_id: draftId, p_image_url: pubUrl.data.publicUrl, p_image_source: "ai_generated" });
