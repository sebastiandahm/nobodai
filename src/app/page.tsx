    ...
    <div style={{ background: "#050709", color: "#e4e4e7", minHeight: "100vh", overflowX: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 2, maxWidth: 860, margin: "0 auto", padding: "80px 28px 60px", textAlign: "center" }}>
          ...
          <div style={{ ...revealStyle(s1.visible, 300) }}>
            <p style={{ fontFamily: "var(--f-body)", fontSize: 17, fontWeight: 300, color: "var(--c-muted)", maxWidth: 500, margin: "0 auto 48px", lineHeight: 1.7 }}>
              An AI that learns how you think, argue, and write &mdash; then drafts LinkedIn posts in your voice and tone, ready to review. Every morning. Automatically.
            </p>
          </div>
          ...
          {/* Feature pills — static, no live-counter suggestion */}
          <div className="stats-row" style={{ ...revealStyle(s1.visible, 600), display: "flex", justifyContent: "center", gap: 32, marginTop: 72, flexWrap: "wrap" }}>
            {[
              { label: "5-step pipeline" },
              { label: "Voice DNA learning" },
              { label: "30 seconds per day" },
            ].map(function (s) {
              return (
                <div key={s.label} style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "8px 16px", borderRadius: 100, border: "1px solid var(--c-border)", background: "rgba(10,12,16,0.6)" }}>
                  <span style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--c-accent)", opacity: 0.8 }} />
                  <span style={{ fontFamily: "var(--f-body)", fontSize: 12, fontWeight: 500, color: "var(--c-muted)", letterSpacing: "0.02em" }}>{s.label}</span>
                </div>
              );
            })}
          </div>
      ...
      <section ref={s2.ref} style={{ maxWidth: 620, margin: "0 auto", padding: "40px 28px 100px" }}>
        ...
        <div style={{ textAlign: "center", marginTop: 14, fontFamily: "var(--f-body)", fontSize: 11, color: "#3f3f46", fontStyle: "italic" }}>
          5-step AI pipeline &bull; Approved in 1 tap
      ...
      <section ref={s4.ref} style={{ maxWidth: 800, margin: "0 auto", padding: "40px 28px 100px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
          {[
            ...
            { n: "3", name: "Deep Draft Generation", desc: "Generates the post using your Voice DNA, proven LinkedIn formats, and tone-matching rules tuned to feel human, not templated.", c: "#f59e0b" },
      ...
      {/* ===== TESTIMONIALS — removed pending verifiable customer quotes with documented consent ===== */}
      ...
      <section id="pricing" ref={s6.ref} style={{ maxWidth: 1100, margin: "0 auto", padding: "80px 28px" }}>
        <div style={{ ...revealStyle(s6.visible), textAlign: "center", marginBottom: 52 }}>
          ...
          <h2 style={{ fontFamily: "var(--f-display)", fontSize: 38, fontWeight: 400, letterSpacing: -1 }}>Pricing that fits how you write.</h2>
          <p style={{ fontFamily: "var(--f-body)", fontSize: 14, color: "var(--c-muted)", marginTop: 8, fontWeight: 300 }}>Start free. Upgrade when you&apos;re ready for daily posts.</p>
        </div>
      ...
      <section ref={s7.ref} style={{ maxWidth: 720, margin: "0 auto", padding: "40px 28px 80px" }}>
        <div className="card" style={{ ...revealStyle(s7.visible), padding: "52px 44px", textAlign: "center", position: "relative", overflow: "hidden", borderColor: "rgba(245,158,11,0.08)" }}>
          ...
          <div className="label" style={{ marginBottom: 8 }}>Built in public</div>
          <h3 style={{ fontFamily: "var(--f-display)", fontSize: 28, fontWeight: 400, marginBottom: 6 }}>Arie Muth</h3>
          <p style={{ fontFamily: "var(--f-body)", fontSize: 13, color: "var(--c-muted)", marginBottom: 20, fontWeight: 300 }}>AI persona &bull; Fully transparent &bull; Posts daily using nobod.ai</p>
          <p style={{ fontFamily: "var(--f-body)", fontSize: 15, color: "#d4d4d8", lineHeight: 1.8, fontStyle: "italic", maxWidth: 480, margin: "0 auto", fontWeight: 300 }}>&ldquo;I&apos;m not human. I don&apos;t pretend to be. The posts you see under my name are written entirely by nobod.ai &mdash; in public, every day. Use it as a working example of what the tool can do.&rdquo;</p>
        </div>
      </section>
      ...
      <section id="faq" ref={s9.ref} style={{ maxWidth: 700, margin: "0 auto", padding: "40px 28px 80px" }}>
        ...
        <div style={{ ...revealStyle(s9.visible, 150), display: "flex", flexDirection: "column", gap: 2 }}>
          {[
            { q: "Isn't this just ChatGPT with a LinkedIn template?", a: "No. nobod.ai runs a 5-step pipeline that goes well beyond a single prompt: Voice DNA analysis of your own writing, topic discovery matched to your expertise, draft generation tuned to feel human and on-brand, a separate AI self-critique that scores and rewrites the draft, and hook variation testing. The output is modeled on your specific patterns and improves with every post you approve or reject \u2014 a generic prompt-based tool can\u2019t do that." },
            { q: "Can people tell it's AI-generated?", a: "Avoiding the obvious AI-feel is exactly what we optimize for. Our system uses varied sentence lengths, concrete personal details, pattern-breaking structures, and learns from every post you approve. The more you use it, the more it sounds like you \u2014 because it is learning from your decisions. We can\u2019t guarantee any specific platform behavior, but we tune for human-feeling output, not for gaming any single algorithm." },
            ...
            { q: "Who is Arie Muth?", a: "Arie is an AI persona we run as a public, working example of nobod.ai. We never claim Arie is human \u2014 every post under that name is generated by the tool. It\u2019s a transparency stance: in a world where AI usage is often hidden, we believe in showing it openly." },
            { q: "How is this different from other AI writing tools?", a: "Three things: First, our 5-step pipeline with self-critique \u2014 a separate AI instance reviews and rewrites the draft before you ever see it. Second, Voice DNA learning that improves with every post you approve or reject. Third, a focus on tone-matching and pattern variation, not template-filling. We aim to be a ghostwriter, not a prompt wrapper." },
      ...
      <footer style={{ borderTop: "1px solid rgba(255,255,255,0.03)", padding: "44px 28px", maxWidth: 1060, margin: "0 auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16 }}>
          <div>
            ...
            <span style={{ fontFamily: "var(--f-body)", fontSize: 12, color: "#27272a", marginLeft: 14 }}>by OPCORE Partners AG, Steinhausen ZG</span>
          </div>
          <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
            <a href="/legal/impressum" style={{ fontFamily: "var(--f-body)", color: "#52525b", fontSize: 12, textDecoration: "none" }}>Impressum</a>
            <a href="/legal/datenschutz" style={{ fontFamily: "var(--f-body)", color: "#52525b", fontSize: 12, textDecoration: "none" }}>Datenschutz</a>
            <a href="/legal/agb" style={{ fontFamily: "var(--f-body)", color: "#52525b", fontSize: 12, textDecoration: "none" }}>AGB</a>
            <a href="/legal/cookies" style={{ fontFamily: "var(--f-body)", color: "#52525b", fontSize: 12, textDecoration: "none" }}>Cookies</a>
